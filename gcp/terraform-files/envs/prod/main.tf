...
resource "google_compute_address" "gke_egress_ip_prod" {
  name   = "gke-egress-ip-prod"
  region = var.region
}
...
module "nat" {
  ...
  nat_name               = "prod-nat-config"
  vpc_network            = module.vpc.network_self_link
  region                 = var.region
  nat_ip_allocate_option = "MANUAL_ONLY"
  nat_ips                = [google_compute_address.gke_egress_ip_prod.self_link]
